<template>
  <div class="model-optimization-page">
    <div class="mb-6">
...
import { 
  Menu, ArrowDown, User, Setting, Close, 
  Plus, RefreshLeft, Loading, Check, ArrowRight 
...
const viewOptimizationDetail = (id: string) => {
  router.push(`/admin/optimization-plans/${id}`)
}

// 编辑优化计划
const editOptimization = (id: string) => {
  router.push(`/admin/optimization-plans/${id}/edit`)
}

// 创建新优化计划
const createNewOptimization = () => {
  router.push('/admin/optimization-plans/new')
}

// 前往对比页面
const gotoComparisonPage = () => {
  router.push('/admin/model-optimization-comparison')
}
</script>
